...
async function fetchData() {
  ...
  let products = await Product.find({category:'cups'});
  let Cups = {};
  for (let item of products) {
    if (item.title in Cups) {
      if (!Cups[item.title].color.includes(item.color) && item.availableQty > 0) {
        Cups[item.title].color.push(item.color);
      }
      if (!Cups[item.title].size.includes(item.size) && item.availableQty > 0) {
        Cups[item.title].size.push(item.size);
      }
    } else {
      Cups[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Cups[item.title].color = [item.color];
        Cups[item.title].size = [item.size];
      }
    }
  }
  return Cups;
  ...
  return (
    ...
    >
      <div className="container mx-auto py-12 px-4">
        {/* <h1 className="text-3xl font-bold text-center mb-8">Featured Cups</h1> */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mx-auto">
          {Object.keys(products).length ===0 && <p>Sorry all Cups are out of stock.Stay Tuned</p>}
