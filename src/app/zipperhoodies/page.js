...
async function fetchData() {
  ...
  let products = await Product.find({category:'ZipperHoodies'});
  let zipperHoodies = {};
  for (let item of products) {
    if (item.title in zipperHoodies) {
      if (!zipperHoodies[item.title].color.includes(item.color) && item.availableQty > 0) {
        zipperHoodies[item.title].color.push(item.color);
      }
      if (!zipperHoodies[item.title].size.includes(item.size) && item.availableQty > 0) {
        zipperHoodies[item.title].size.push(item.size);
      }
    } else {
      zipperHoodies[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        zipperHoodies[item.title].color = [item.color];
        zipperHoodies[item.title].size = [item.size];
      }
    }
  }
  return zipperHoodies;
}
  ...
  return (
    ...
    >
      <div className="container mx-auto py-12 px-4">
        {/* <h1 className="text-3xl font-bold text-center mb-8">Featured zipperHoodies</h1> */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mx-auto">
          {Object.keys(products).length ===0 && <p>Sorry all zipperHoodies are out of stock.Stay Tuned</p>}
